      ...
      <ul class="spec-list">
        <li class="item" v-for="(item, index) in specification" :key="index">
          ...
          <div class="values">
            ...
            <div class="add-attr">
              <el-input size="small" v-model="addValues[index]" placeholder="多个产品属性以空格隔开" @keyup.native.enter="addSpecTag(index)">
                <el-button slot="append" icon="el-icon-plus" @click="addSpecTag(index)"></el-button>
              </el-input>
            </div>
          </div>
        </li>
