<template>
  <div class="error-page">
    <div class="error-pos">
      ...
      <div class="main">
        <h1>Error</h1>
        <p>您迷路了！<a href="javascript:void(0)" @click="$router.go(-1)" class="goback">返回上一页</a>/<a href="/" class="goback">返回首页</a></p>
        <p>您将在{{ time }}秒后返回主页</p>
      </div>
    </div>
  </div>
...
<style scoped lang="scss">
  ...
  .goback{font-size:12px;color:#fff;}
  .error-page{position:fixed;height:100%;width:100%;background:linear-gradient(#203075,#233581);overflow:hidden;
    &:before{content:'';background:linear-gradient(#233581,#203075);border-radius:50%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);height:105vmax;width:105vmax;z-index:-4}
    &:after{content:'';background:linear-gradient(#203075,#233581);border-radius:50%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);height:80vmax;width:80vmax;z-index:-3}
    .error-pos{height:100vh;width:100%;position:relative;display:flex;justify-content:center;align-items:center;color:#FFF;font-family:'Varela Round',Sans-serif;text-shadow:0 30px 10px rgba(0,0,0,0.15);
      &:before{content:'';background:linear-gradient(#233581,#203075);border-radius:50%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);height:60vmax;width:60vmax;z-index:-2}
      &:after{content:'';background:linear-gradient(#203075,#233581);border-radius:50%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);height:40vmax;width:40vmax;z-index:-1}
    }
  }
</style>
